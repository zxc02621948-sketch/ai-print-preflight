...
// Set the effective-DPI target from "output size -> typical viewing distance".
// The eye's resolving power is fixed, so the bigger the print and the farther you
// stand, the lower the DPI you actually need. Small things viewed up close (cards,
// stickers) need 300; an A4/A3 on a desk is fine around 200; posters and banners
// seen from 1+ meters look sharp at ~100 or even less.
// The old code demanded 300 regardless of distance, which wrongly flagged medium/
// large sizes as "blurry" — this version grades by distance.
...
// Always compute in mm internally; inputs can switch mm / cm. This factor
// converts the entered value back to mm.
function unitFactor() {
...
els.unit.addEventListener("change", () => {
  // Convert the current values so the physical size stays the same (30mm ↔ 3cm).
...
async function loadFile(file) {
  ...
  image.onload = async () => {
    ...
    els.infoOrigDpi.textContent = origDpi ? `${origDpi} DPI` : "Not tagged";
...
// Read the file's embedded DPI tag (the "Resolution" field you see in Photoshop).
// Note: this is just metadata and does NOT affect print sharpness — sharpness is the
// effective DPI (pixels / output size).
async function readEmbeddedDpi(file) {
  try {
    ...
    /* if it can't be read, treat as no tag */
  }
  return null;
}
...
function renderPrintSim(metrics) {
  ...
  // Size the preview box to the OUTPUT size ratio (not the source image ratio),
  // so users see the image printed into that shape. Both <img> layers use
  // object-fit: fill, so a square image dropped into a rectangle deforms to match.
  ...
  // The "how it prints" layer uses downsample-then-upscale to mimic real low resolution:
  // too few dots = fine detail softens, but big shapes / edges stay crisp — matching real printing.
  // No more Gaussian blur (it smears the whole image uniformly, far foggier than a real print).
  // When DPI is enough, use the original image so both layers are pixel-for-pixel identical.
...
// Shrink the image to its real deliverable detail (ratio x display size), then scale back up.
// Big shapes stay crisp and only fine detail softens — closer to real low-res print than Gaussian blur.
...
function upscaleFix(metrics) {
  ...
  const steps = [
    "<strong>No-install route (easiest for anime / illustration):</strong> pick your image and a scale factor (try 2x first), then download the PNG — it's best-in-class for anime / illustration.<br><a href=\"https://bigjpg.com/en\" target=\"_blank\" rel=\"noopener\" style=\"display:inline-block;margin:12px 0;padding:11px 20px;background:#0f766e;color:#fff;border-radius:10px;text-decoration:none;font-weight:800;font-size:1.02em;\">🔗 Open Bigjpg — free upscaler</a><br>Note: free tier caps at 3000×3000px / 5MB and it uploads your image (China server by default, auto-deleted within 24h). <strong>If your image is realistic / photo-style, or larger, or you'd rather not upload, use the Upscayl route below.</strong>",
